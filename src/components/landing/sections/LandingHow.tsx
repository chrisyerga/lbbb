...
const TAPE_COLORS = ['var(--landing-accent)', 'var(--landing-soft)', 'var(--landing-primary)'] as const
const ROTATIONS = [-1.5, 1, -0.5] as const
const TAPE_ROTATIONS = [-12, 8, -4] as const

export function LandingHow() {
  return (
    <section id="how-it-works" className="relative px-4 py-16 sm:px-8 sm:py-24 lg:px-14 lg:py-[120px]">
      ...
      <div className="grid gap-7 md:grid-cols-3 md:gap-7">
        {HOW_STEPS.map((s, i) => (
          ...
          >
            <Tape color={TAPE_COLORS[i]} w={70} rotate={TAPE_ROTATIONS[i]} className="absolute -top-2.5 left-6" />
            ...
            <p className="m-0 text-[15.5px] leading-normal text-[var(--landing-ink)]/80">{s.d}</p>
          </div>
        ))}
      </div>
    </section>
