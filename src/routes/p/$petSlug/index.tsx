...
function PublicPetBlog() {
  ...
  const examplePost = publicRoutes.petPost(petSlug, '2026-05-07-the-great-squirrel-standoff')
  ...
  return (
    <main className="page-wrap px-4 pb-12 pt-8">
      <article className="panel p-8">
        ...
        <h1 className="m-0 text-4xl font-bold text-[var(--text-primary)]">{blog.title}</h1>
        <p className="mt-2 text-xl font-semibold text-[var(--text-primary)]">{pet.name}</p>
        {blog.description ? (
        ...
        {pet.bio ? <p className="max-w-2xl text-sm text-[var(--text-muted)]">{pet.bio}</p> : null}
