...
export function useCastPhotoUpload(castMemberId: Id<'castMembers'>) {
  const generateUploadUrl = useMutation(api.assets.generateUploadUrl)
  const finalizeUpload = useMutation(api.assets.finalizeCastReferenceUpload)
  ...
  const uploadFile = useCallback(
    ...
    [castMemberId, finalizeUpload, generateUploadUrl],
  )
